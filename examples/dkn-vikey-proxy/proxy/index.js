...
if (!VIKEY_API_KEY) {
  console.error('VIKEY_API_KEY kosong. Set di .env');
  process.exit(1);
}

// Target TPS minimal agar lolos pemeriksaan awal DKN
...
// Estimasi kasar jumlah token (±4 chars/token)
...
// Health dengan versi agar memudahkan pengecekan
app.get('/health', (_req, res) => {
  res.json({ ok: true, proxy: 'ollama2vikey', version: '0.2.0', min_tps: MIN_TPS });
});

// Minimal /api/tags agar DKN melihat daftar model
...
app.post('/api/generate', async (req, res) => {
  ...
  try {
    ...
    // Hitung TPS dan boost jika di bawah MIN_TPS
